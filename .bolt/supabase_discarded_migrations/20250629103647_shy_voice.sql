...
SELECT 
  ...
  p.sku || '-' || size_data.size || '-' || color_data.color_code,
  50, -- Default stock
  true
FROM products p
CROSS JOIN (
  ...
  WHERE p.category_id IN (
    SELECT id FROM categories WHERE slug IN ('mens-t-shirts', 'mens-bottomwear', 'night-wear')
  )
  UNION ALL
  SELECT unnest(ARRAY['S', 'M', 'L', 'XL', 'XXL', '3XL', '4XL', '5XL']) as size
  WHERE p.category_id IN (
    SELECT id FROM categories WHERE slug IN ('womens-leggings', 'saree-shapewear')
  )
) size_data
CROSS JOIN (
  VALUES 
    ...
    ('Purple', '#800080'),
    ('Royal', '#4169E1')
) color_data(color_name, color_code)
WHERE NOT EXISTS (
  SELECT 1 FROM product_variants pv 
  WHERE pv.product_id = p.id 
  AND pv.size = size_data.size 
  AND pv.color_code = color_data.color_code
)
LIMIT 5; -- Limit to 5 color variants per size to avoid too many combinations